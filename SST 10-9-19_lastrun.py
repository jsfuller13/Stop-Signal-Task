...
expName = 'SST 10-9-19'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Jordan\\Documents\\Experiment Building\\3. Stop Signal Task\\SST 10-9-19_lastrun.py',
...
for thisB2_trial in B2_trials:
    ...
    while continueRoutine:
        ...
        if B2_resp.status == STARTED:
            ...
            if len(theseKeys):
                ...
                continueRoutine = False
        # start/stop signal
        if t >= mean_RT-0.225 and signal.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        frameRemains = mean_RT-0.225 + 0.1- win.monitorFramePeriod * 0.75  # most of one frame period left
    ...
    thisExp.addData('B1_mean_rt', mean_RT)
    ...
    # the Routine "B2_Trials" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    thisExp.nextEntry()
