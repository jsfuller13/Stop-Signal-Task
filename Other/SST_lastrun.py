...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\Jordan\\Documents\\Experiment Building\\3. Stop Signal Task\\SST_lastrun.py',
...
B2_stim_text = visual.TextStim(win=win, name='B2_stim_text',
    ...
    languageStyle='LTR',
    depth=0.0);
B2_fixation = visual.TextStim(win=win, name='B2_fixation',
    text='+',
    font='Arial',
    pos=(0, 0), height=0.1, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
...
for thisB1_trial in B1_trials:
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *B1_stim_txt* updates
        if t >= 1 and B1_stim_txt.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        # *B1_resp* updates
        if t >= 1 and B1_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
    ...
    B1_trials.addData('B1_resp.started', B1_resp.tStartRefresh)
    B1_trials.addData('B1_resp.stopped', B1_resp.tStopRefresh)
    thisExp.nextEntry()
...
# set up handler to look after randomisation of conditions etc
B2_trials = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
...
for thisB2_trial in B2_trials:
    ...
    # keep track of which components have finished
    B2_TrialsComponents = [B2_stim_text, B2_resp, B2_fixation]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        if t >= 1 and B2_stim_text.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        # *B2_resp* updates
        if t >= 1 and B2_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        # *B2_fixation* updates
        if t >= 0.5 and B2_fixation.status == NOT_STARTED:
            # keep track of start time/frame for later
            B2_fixation.tStart = t  # not accounting for scr refresh
            B2_fixation.frameNStart = frameN  # exact frame index
            win.timeOnFlip(B2_fixation, 'tStartRefresh')  # time at next scr refresh
            B2_fixation.setAutoDraw(True)
        frameRemains = 0.5 + 0.5- win.monitorFramePeriod * 0.75  # most of one frame period left
        if B2_fixation.status == STARTED and t >= frameRemains:
            # keep track of stop time/frame for later
            B2_fixation.tStop = t  # not accounting for scr refresh
            B2_fixation.frameNStop = frameN  # exact frame index
            win.timeOnFlip(B2_fixation, 'tStopRefresh')  # time at next scr refresh
            B2_fixation.setAutoDraw(False)
    ...
    for thisComponent in B2_TrialsComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    B2_trials.addData('B2_stim_text.started', B2_stim_text.tStartRefresh)
    B2_trials.addData('B2_stim_text.stopped', B2_stim_text.tStopRefresh)
    # check responses
    ...
    B2_trials.addData('B2_resp.started', B2_resp.tStartRefresh)
    B2_trials.addData('B2_resp.stopped', B2_resp.tStopRefresh)
    B2_trials.addData('B2_fixation.started', B2_fixation.tStartRefresh)
    B2_trials.addData('B2_fixation.stopped', B2_fixation.tStopRefresh)
    thisExp.nextEntry()
